function [C_ultimate, R_over_D,J_fairness,mean_R_all]=proposedMethodwoFragmentation(h_vector, h_vector_sqr, epsilon_cell, data, center_data, num_cells, num_sats, sigma2, Ptx, ~, Demand, wgs84,nbrOfRB)

% Configure bandwidth and per-satellite beam limits.
B=nbrOfRB*180*1e3;
...
function patterns = recursivePatternGeneration(num_cells, active_cells, potential_active_cells, a_var, adjacentList, adjacentList2, Demand, epsilon_cell, h_vector_sqr, h_vector, w, gamma, y, sigma2, Ptx, max_depth,thresholds,depth, d, J,...
       eta_threshold)
    ...
    for cell_idx = potential_active_cells
        ...
        [a_new_cell, feasible] = assignBeams(cell_idx, a_var, Demand, epsilon_cell, h_vector_sqr, h_vector, w, y, sigma2, Ptx, active_cells, eta_threshold);
        ...
        for cID = new_active_cells
            ...
            denominator_cID = interference_cID + sigma2/Ptx;
        ...
        Rate_comparison = w(sc) * log2(1 + gamma(sc));
        eta_value = Rate_comparison / Demand(sc);
        sub_patterns = recursivePatternGeneration(num_cells, new_active_cells, new_potential_active_cells, new_a_var, adjacentList, adjacentList2, Demand, epsilon_cell, h_vector_sqr, h_vector, w, gamma, y, sigma2, Ptx, ...
...
function [a_m_new, feasible, gamma_m_new, y_m_new] = assignBeams(m, a_var, Demand, epsilon_cell, h_vector_sqr, h_vector, w,  y, sigma2, Ptx, active_cells, eta_threshold)
    ...
    for n = active_cells
        ...
        denominator_n = interference_n + sigma2 / Ptx;
...
for c = 1:group_num
    ...
    max_depth = ceil(length(groups{c})/4);
    depth_initial = 1;
...
y,sigma2, Ptx, max_depth, thresholds, depth_initial, d, J,eta_threshold);
...
for v = 1:total_patterns
    ...
    for m = active_cells
        ...
        denominator(m) = interference + sigma2 / Ptx;
        gamma_var(m) = numerator(m) / denominator(m);
        Rate_matrix_ideal(m, v) = w(m) * log2(1 + numerator(m) / (sigma2 / Ptx));
